...
\begin{itemize}
    \item Manages the AI conversation lifecycle, including creating, listing, renaming, retrieving, and soft-deleting conversations.
    \item Stores conversation metadata and message histories in the database, allowing users to continue previous chat sessions.
    \item Acts as an adapter between HTTP controllers and the external n8n AI workflow by sending user messages to the configured n8n webhook.
    \item Supports streaming chatbot responses to the client using Server-Sent Events (SSE), improving the real-time interaction experience.
    \item Handles chatbot-related access control by ensuring that users can only access and modify their own conversations.
\end{itemize}
...
\subsubsection{Contest Module}
\begin{itemize}
    \item Create and manage programming contests (problems, schedules, participants).
    \item Handle contest registration, team management, and participant tracking.
    \item Provide real-time ranking and leaderboard updates during contests.
    \item Enforce contest rules and handle edge cases (e.g., late submissions, ties).
\end{itemize}

...
Once deployed, the frontend is served via Vercel and communicates with the backend service exposed from Google Cloud.
...
The backend API is deployed as a Kubernetes Deployment on Google Kubernetes Engine. The backend is implemented using NestJS and is packaged into a Docker image before deployment. The build process installs production dependencies, compiles the TypeScript source code, and produces a runnable Node.js application. The resulting image is pushed to Google Artifact Registry and referenced by the Kubernetes Deployment manifest.
...
The backend service is responsible for handling authentication, problem management, submission creation, discussion features, and result retrieval. During submission creation, the backend stores the submission in PostgreSQL and publishes an evaluation request to RabbitMQ. The message payload contains the submission identifier, programming language, submitted source code, testcase URL, timeout, memory limit, and entry function. This makes the worker independent from the backend database when executing a submission.

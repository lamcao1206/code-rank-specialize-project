...
\subsection{Chatbot Service Architecture}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Contents/Chapter 3/Chatbot Design/chatbot_architecture.pdf}
    \caption{Entity Relationship Diagram}
\end{figure}

\begin{enumerate}
   \item \textbf{Chatbot Service}
The Chatbot Service represents the entire operational boundary of the AI interview coach. It contains all the core logic, memory systems, agent tools, and integrations required to facilitate technical interview-style conversations.
   \item \textbf{Context Base}
The Context Base is responsible for storing and retrieving conversational context during an interview session. It uses LangChain’s built-in short-term memory mechanism, which relies on a checkpointer to save and load the recent conversation state for each thread.

\textbf{Short-Term Context Manager}
\begin{itemize}
   \item Maintains the latest messages in the conversation (short-term only).
   \item Prevents the prompt from growing too large by only storing recent turns.
   \item Uses LangChain’s state-based memory, managed through a checkpointer \texttt{PostgresSaver}
\end{itemize}
This ensures the agent has “active context” for follow-up questions and code explanation steps.

   \item \textbf{Prompt Builder}
\begin{itemize}
   \item Constructs structured prompts according to interview rules defined in the System Prompt.
   \item Integrates: user input, interview mode instructions and short-term context from the Context Base.
   \item Uses LangChain’s \textit{PromptTemplate} and \textit{ChatPromptTemplate} to dynamically shape the prompt.
\end{itemize}

   \item \textbf{Response Processor}
\begin{itemize}
   \item Receives the raw output from the LLM and converts it into a clean, structured reply appropriate for interview interaction.
   \item Applies LangChain output parsers if needed.
\end{itemize}
   \item \textbf{Service Consumers, Publishers}

   This intermediary layer links the Chatbot Service with main backend systems. It handles incoming requests from the Backend Application and routes them to the Chatbot Service. It also publishes the chatbot’s responses back to the Backend Application.
